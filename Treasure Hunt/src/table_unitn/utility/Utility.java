	...
	public static int calcolakm(String res, int n_tappa) throws JSONException{
      ...
      //arccos(cos(a1-a2)cos(b1)cos(b2)+sin(b1)sin(b2))
        ...
        for(int i=1;i<mySArray.length;i++){
        	...
        	Double perarcos=Math.cos(ll0[0]-ll1[0])*Math.cos(ll0[1])*Math.cos(ll1[1])+Math.sin(ll0[1])*Math.sin(ll1[1]);
        	dist+= Math.acos(perarcos);
        	
        }
        dist=dist*6378;
		return dist.intValue(); //il valore calcolato non è corretto
	}
}
